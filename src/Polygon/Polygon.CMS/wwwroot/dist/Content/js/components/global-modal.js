var Modal = Modal ||
    {
        Initialize: function () {
            $(function () {
                $("body").on("click", "#global-modal", function (e) {
                    var button = $(this);
                    e.preventDefault();
                    Modal.ShowWindow(button.attr("href") ? button.attr("href") : button.data("url"), button.data("size"));
                });
                $("#global-modal .modal-content").on("click", ".close-modal", function (e) {
                    Modal.CloseWindow();
                });
        ...
        SetupForm: function (statusCodeActions) {
            $("#global-modal").on("submit", "form", function (e) {
                e.preventDefault();
                if ($(this).valid()) {
                    $.ajax({
                        method: "POST",
